...
require_once 'includes/chatgpt_analyzer.php';
...
try {
    ...
    $analyzer = new ChatGPTAnalyzer($db);

    // Criar tabela de análise se não existir
    $sqlFile = __DIR__ . '/sql/create_ai_analysis_table.sql';
...
<body>
                                            ...
                                            <div class="card test-card">
                                                <div class="card-body">
                                                    ...
                                                    <button class="btn btn-primary view-results" 
                                                            ...
                                                            data-analysis='<?php 
                                                                try {
                                                                    $analysis = $analyzer->analyzeTestResults(
                                                                        $result['test_type'], 
                                                                        json_decode($result['results'], true),
                                                                        $result['id'] // Passando o ID do resultado do teste
                                                                    );
                                                                    echo htmlspecialchars($analysis, ENT_QUOTES);
                                                                } catch (Exception $e) {
                                                                    error_log("Erro na análise do ChatGPT: " . $e->getMessage());
                                                                    echo '';
                                                                }
                                                            ?>'>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.view-results').forEach(button => {
                button.addEventListener('click', function() {
                    try {
                                ...
                                content = `
                                    <div class="table-responsive mb-4">
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ${Object.entries(counts).map(([dim, score]) => `
                                                    <tr>
                                                        <td>${dim}</td>
                                                        <td>${score}%</td>
                                                        <td>${interpretDiscScore(score)}</td>
                                                    </tr>
                                                `).join('')}
                                            </tbody>
                                        </table>
                        ...
                        // Adicionar a análise do ChatGPT se disponível
                        if (analysis) {
                            content += `
                                <div class="card mt-4">
                                    <div class="card-header">
                                        <h5 class="mb-0">Análise Detalhada (IA)</h5>
                                    </div>
                                    <div class="card-body">
            ...
            // Função auxiliar para interpretar pontuações do DISC
            function interpretDiscScore(score) {
                if (score <= 25) return 'Baixo';
                if (score <= 50) return 'Moderado Baixo';
                if (score <= 75) return 'Moderado Alto';
                return 'Alto';
            }
